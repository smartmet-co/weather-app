...
import { GREEN, ORANGE, YELLOW } from '../../src/assets/colors';
...
describe('WarningIcon', () => {
  ...
  it('returns null when neither severity nor severityDescription is provided', () => {
    const { toJSON } = render(<WarningIcon type="Severe weather warning" />);
  ...
  it('renders severity circle color for numeric severity and hides icon when severity is 0', () => {
    const { toJSON, queryByTestId } = render(<WarningIcon type="Severe weather warning" severity={0} />);
  ...
  it('maps severityDescription to severity color and icon name', () => {
    const { toJSON, getByTestId } = render(
      <WarningIcon type="Tropical storm advisory" severityDescription="Severe" />
    );
    ...
    expect(mockIcon).toHaveBeenCalledWith(
      expect.objectContaining({
        name: 'fmi-warnings-Tropical storm advisory',
  ...
  it('uses yellow color for Moderate severityDescription', () => {
    const { toJSON } = render(<WarningIcon type="Landslide warning" severityDescription="Moderate" />);
    const tree = toJSON() as any;
    const containerStyle = getMergedStyle(tree.props.style);
